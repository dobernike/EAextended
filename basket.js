...
if (profile.textContent.trim() !== "П") {
  ...
  const totalCartPrice = document.querySelector('#total-cart-price').textContent;

  // if (+totalCartPrice !== 0) {
  if (false) {

    const cartStoreWraps = document.querySelectorAll('.cart-store-wrap');
    for (const cartStoreWrap of cartStoreWraps) {
      changeCartStore(cartStoreWrap);
    }

    function changeCartStore(cartStoreWrap) {
      const cartStoreName = cartStoreWrap.dataset.sellerId;
      const cartItemProp = cartStoreWrap.querySelector('.cart-item-prop');

      // localStorage.setItem('newItem', 'test');
      // chrome.storage.local.set({ 'newItem': 'test' }, function () {
      //   alert('set');
      // });

      // console.log(cartItemProp, cartItemProp.href, getFetchPrice(cartItemProp.href));

      // getFetchPrice(cartItemProp.href);
      // const delay = ms => new Promise(resolve => setTimeout(resolve, ms));

      // async function getFetchPrice(url) {
      //   console.log(url + '?view=1');
      ...
      if (cartStoreName === 'lc_new') {
        // saf
      }

      if (cartStoreName === 'ldc') {
        // saf
      }
    }

    // chrome.storage.local.get(['newItem'], function (newItem) {
    //   alert('get' + ' ' + Object.values(newItem))
    // })
